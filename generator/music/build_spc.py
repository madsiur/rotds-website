...
def build_spc(rom, song_idx, path=None):
    
    # SPC format - 64KB memory + $100 bytes for DSP registers etc. at end
    spc = bytearray(0x10100)
    
    spc_work_ram = load_binary_file(os.path.join(path, SPC_WORK_RAM_FILE), expected_size = 0x300)
    spc_aux_ram = load_binary_file(os.path.join(path, SPC_AUX_RAM_FILE), expected_size = 0xB00)
